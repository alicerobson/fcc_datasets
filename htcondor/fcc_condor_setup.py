...
''' fcc_condor_setup.py is used to setup all files and directories needed for a condor batch run
    It should normally be launched via fcc_condor_submit.sh
    
    Usage: fcc_condor_setup.py  -p parameters_yaml_file -e events -r runs
    
    Example:
    fcc_condor_setup.py -p papas_parameters.yaml
    Tutorial:
    #Source init.sh for
    - FCCSW
    - heppy
    ...
    fcc_condor_setup.py -p papas_parameters.yaml
    #bigger run
    fcc_condor_setup.py -p papas_parameters.yaml -e 100 -r 4
    ...
    Some submission files are written/added to by fcc_condor_setup.py according to run parameters
...
def write_condor_software_yaml(subdir, filename="software.yaml"):
    ...
    It uses the env variables to search for software.move_xrdp.
